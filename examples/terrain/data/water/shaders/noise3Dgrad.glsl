...
//
// Adjusted for Castle Game Engine by Michalis Kamburelis, 2026-2026.
// - We need highp precision for WebGL.
//   Testcase: otherwise noise for large terrain values gets flat,
//   resulting in dark flat water surface on examples/terrain/
//   (on Chrome/Vivaldi, not on Firefox).
...
// The ring size 17*17 = 289 is close to a multiple of 49 (49*6 = 294)
  ...
  //highp vec4 s0 = highp vec4(lessThan(b0,0.0))*2.0 - 1.0;
  //highp vec4 s1 = highp vec4(lessThan(b1,0.0))*2.0 - 1.0;
...
// Mix final noise value
  ...
  highp vec4 m2 = m * m;
  highp vec4 m4 = m2 * m2;
  highp vec4 pdotx = vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3));

// Determine noise gradient
  highp vec4 temp = m2 * m * pdotx;
