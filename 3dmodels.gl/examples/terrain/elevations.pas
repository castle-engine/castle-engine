...
type
  ...
  { Elevation data from a nice noise.

    "Nice noise" means it's not simply something random,
    it's in fact several "octaves" (simpler noise functions,
    with frequency and amplitude), each one smoothed by some interpolation,
    and added together. This is the kind of noise used to synthesize textures,
    ...
    @unorderedList(
      @item(
        The simplest introduction I found is on
        http://freespace.virgin.net/hugo.elias/models/m_perlin.htm
        It describes how to get nice noise very easily, and my approach follows
        theirs.

        Note: one thing I'm not sure about the tutorial above it whether
        it really describes Perlin noise,
        Perlin noise is just a method for calculating it from gradients
      ...
      @item(http://en.wikipedia.org/wiki/Fractional_Brownian_motion
        Also Ken Musgrave's dissertation:
...
{ Noise for @D coords, resulting in float 0..1 range.
  ...
  TODO: for now no interpolation...  }
function InterpolatedNoise2D(const X, Y: Single; const NoiseIndex: Cardinal): Single;
begin
...
begin
  ...
  begin
    Result += InterpolatedNoise2D(X * Frequency, Y * Frequency, I) * Amplitude;
    Frequency *= 2;
