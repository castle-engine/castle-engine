...
type
  ...
  public
    ...
    { Use shorter directory names, sacrificing readability, to avoid long
      file paths on some platforms (e.g. Windows). }
    property ShortDirNames: Boolean read FShortDirNames;
...
begin
  ...
  FShortDirNames := Element.AttributeBooleanDef('short_dir_names', false);
...
begin
  Result := false;
  SplitPathAndMask(Url, UrlPath, UrlName);
  for I := 0 to IncludePaths.Count - 1 do
  begin
    SplitPathAndMask(IncludePaths[I], IncludePath, IncludeMask);
    if IncludePathsRecursive[I] then
      PathMatches := IsPrefix(IncludePath, UrlPath, PathsIgnoreCase)
    else
...
function TAutoGeneratedTextures.GeneratedTextureUrl(
  ...
  const Scaling: Cardinal): String;
const
  SUncompressed: array [{ ShortDirNames } Boolean] of String = ('uncompressed', 'u');
  SDownscaled: array [{ ShortDirNames } Boolean] of String = ('downscaled_', 's');
var
  UrlPath, UrlName: String;
begin
  SplitPathAndMask(Url, UrlPath, UrlName);
  Result := UrlPath + TAutoGenerated.AutoGeneratedDirName + '/';
  if UseCompression then
    Result := Result + LowerCase(TextureCompressionToString(TextureCompression)) + '/'
  else
    Result := Result + SUncompressed[ShortDirNames] + '/';
  if Scaling <> 1 then
    Result := Result + SDownscaled[ShortDirNames] + IntToStr(Scaling) + '/';
