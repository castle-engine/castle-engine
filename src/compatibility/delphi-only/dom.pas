...
uses XMLDoc, XMLIntf, Contnrs, Classes, SysUtils, Variants;
...
begin
  { This can happen when XML has value like original_platforms="".
    ...
    Testcase: explore_impressive_castle with data/CastleAutoGenerated.xml
    (and various auto-tests in TTestCastleXmlUtils) }
  if VarIsNull(InternalNode.NodeValue) then
    Result := ''
  else
...
begin
  ...
  begin
    { This can happen when XML has value like original_platforms="".
      ...
      "EVariantTypeCastError: Could not convert variant of type (Null) into type (OleStr)"
      Testcase: explore_impressive_castle with data/CastleAutoGenerated.xml
      (and auto-test TTestCastleXmlUtils.TestEmptyAttribute ) }
    if VarIsNull(ANodes[Index].NodeValue) then
      Value := ''
    else
