...
type
  ...
  { Description of a single "scale" entry:
    Value - The scale (as in ScaledWidth := Width / (2 ^ Scale.Value).
    ...
    Platforms - list of platforms for which this scale should be packaged. }
  TScale = record
    Value: Byte;
    Platforms: TCastlePlatforms;
  end;
  TScalesArray = array of TScale;
  ...
  strict private
    type
      ...
      public
        ...
        { Set of TScale records for this auto-generated texture. Read-only. }
  ...
  public
    ...
    function OriginalPlatforms(const TextureURL: String): TCastlePlatforms;

    { Generate all scales for this TextureURL.
      @param(TextureURL An absolute texture URL. Usually should just be taken
        from AutoGeneratedTextures returned list.) }
    function Scales(const TextureURL: String): TScalesArray;
...
constructor TMaterialProperties.TAutoGeneratedTextures.Create(
  ...
  function StrToScale(AString: String): Cardinal;
  begin
    Result := StrToInt(AString);
    // TODO : more specific exception types
    if Result < 1 then
      raise Exception.Create('Error reading scale value "' + AString + '", the scale value must be >=1.');
    if (Result > 32) then
      raise Exception.Create('Error reading scale value "' + AString + '", the scale value cannot be larger than 32.');
  end;

var
  ChildElements: TXMLElementIterator;
  ChildElement, CompressElement, ScalesElement, PlatformsElement, PreferredOutputFormatElement: TDOMElement;
...
begin
  ...
  begin
    { Deprecatd "scale" element }
    WriteLnWarning('AutoGeneratedTextures', 'MaterialProperties contains "auto_generated_texture/scale" node which is deprecated.');
    ...
    begin
      ...
      if (ASmallestScale < 1) or (ASmallestScale > 32) then
        // TODO: more specific exception type
        raise Exception.CreateFmt('Invalid scale smallest value "%d" (must be an integer number within 1..32)', [ASmallestScale]);
...
function TMaterialProperties.TAutoGeneratedTextures.LargestScaleValue: Byte;
var
  Scale: TScale;
begin
  Result := 1;
...
function TMaterialProperties.OriginalPlatforms(const TextureURL: String): TCastlePlatforms;
var
  Info: TAutoGeneratedTextures;
  Scale: TScale;
begin
...
function TMaterialProperties.Scales(const TextureURL: String): TScalesArray;
var
  Info: TAutoGeneratedTextures;
  I: Integer;
begin
  ...
  if Info <> nil then
  begin
    SetLength(Result, Info.Scales.Count);
    for I := 0 to Info.Scales.Count - 1 do
      Result[I] := Info.Scales[I];
  end;
end;
