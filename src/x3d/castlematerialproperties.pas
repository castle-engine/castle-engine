...
type
  ...
  strict private
    type
      ...
      strict private
        ...
        FBaseURL: string;
        FCompressedFormats: TTextureCompressions;
      ...
      public
        ...
        class function GeneratedTextureURL(const URL: string;
          const UseCompression: boolean; const TextureCompression: TTextureCompression;
          const Scaling: Cardinal): string;
        property CompressedFormats: TTextureCompressions read FCompressedFormats;
        property SmallestScale: Cardinal read FSmallestScale;
      end;
    var
    FAutoGeneratedTextures: TAutoGeneratedTextures;
    FURL: string;
    FAutoProcessImageURLs: boolean;
  ...
  public
    ...
    { For given texture (absolute) URL and compression and scaling,
      return the proper URL of auto-compressed and auto-downscaled counterpart.
      Scaling is defined just like TextureLoadingScale. }
    class function AutoGeneratedTextureURL(const TextureURL: string;
      const UseCompression: boolean;
      const TextureCompression: TTextureCompression; const Scaling: Cardinal): string;

    { Automatic compression formats used for all AutoGeneratedTextures. }
    function AutoCompressedTextureFormats: TTextureCompressions;

    { Automatically generated smallest scale. Value intepreted just like TextureLoadingScale.
      See http://castle-engine.sourceforge.net/creating_data_material_properties.php#section_texture_scale }
    function AutoScale: Cardinal;
  end;
...
var
  { Use the auto-generated alternative downscaled images.
    ...
    See the http://castle-engine.sourceforge.net/creating_data_material_properties.php#section_texture_scale .
...
var
  ...
  ChildElement, FormatsElement, ScaleElement: TDOMElement;
begin
  inherited Create;
  ...
  begin
    ...
    try
      while ChildElements.GetNext do
        Include(FCompressedFormats, StringToTextureCompression(
...
begin
  if (Pos('/' + AutoGeneratedDirName + '/', FileInfo.URL) = 0) and
...
var
  ...
  begin
    ...
    if GLFeatures = nil then
      OnWarning(wtMinor, 'MaterialProperties', 'Cannot determine whether to use GPU compressed (and potentially downscaled) version for ' + URL + ' because the image is loaded before GPU capabilities are known') else
    for C in CompressedFormats do
      if C in GLFeatures.TextureCompression then
      begin
        URL := GeneratedTextureURL(URL, true, C, TextureLoadingScale);
        WritelnLog('MaterialProperties', 'Using compressed (and potentially downscaled) alternative ' + URL);
        Exit;
      end;
    { no GPU compression supported; still, maybe we should use a downscaled alternative }
    if TextureLoadingScale <> 1 then
    begin
      URL := GeneratedTextureURL(URL, false, Low(TTextureCompression), TextureLoadingScale);
      WritelnLog('MaterialProperties', 'Using downscaled alternative ' + URL);
    end;
  end;
...
class function TMaterialProperties.TAutoGeneratedTextures.GeneratedTextureURL(
  const URL: string;
  const UseCompression: boolean; const TextureCompression: TTextureCompression;
  const Scaling: Cardinal): string;
begin
  Result := ExtractURIPath(URL) + AutoGeneratedDirName + '/';
  if UseCompression then
    Result += LowerCase(TextureCompressionToString(TextureCompression)) + '/' else
    Result += 'uncompressed/';
  if Scaling <> 1 then
    Result += 'downscaled_' + IntToStr(Scaling) + '/';
...
begin
  FreeAndNil(FAutoGeneratedTextures);
  inherited;
end;
...
var
  ...
  Stream: TStream;
  AutoGeneratedTexturesElement: TDOMElement;
begin
  FURL := Value;
  ...
  try
    ...
    AutoGeneratedTexturesElement := Config.DocumentElement.ChildElement('auto_generated_textures', false);
    if AutoGeneratedTexturesElement <> nil then
      FAutoGeneratedTextures := TAutoGeneratedTextures.Create(
        AutoGeneratedTexturesElement, URL, FAutoProcessImageURLs);
  finally
...
begin
  if FAutoGeneratedTextures <> nil then
    Result := FAutoGeneratedTextures.AutoGeneratedTextures else
    Result := TCastleStringList.Create;
end;

class function TMaterialProperties.AutoGeneratedTextureURL(const TextureURL: string;
  const UseCompression: boolean;
  const TextureCompression: TTextureCompression; const Scaling: Cardinal): string;
begin
  Result := TAutoGeneratedTextures.GeneratedTextureURL(TextureURL,
    UseCompression, TextureCompression, Scaling);
end;

function TMaterialProperties.AutoCompressedTextureFormats: TTextureCompressions;
begin
  if FAutoGeneratedTextures <> nil then
    Result := FAutoGeneratedTextures.CompressedFormats else
    Result := [];
end;

function TMaterialProperties.AutoScale: Cardinal;
begin
  if FAutoGeneratedTextures <> nil then
    Result := FAutoGeneratedTextures.SmallestScale else
    Result := 1;
end;
