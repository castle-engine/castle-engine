...
type
  ...
  strict private
    type
      ...
      TAutoGeneratedTexturesList = specialize TFPGObjectList<TAutoGeneratedTextures>;
      TMaterialPropertyList = specialize TFPGObjectList<TMaterialProperty>;
    var
  ...
  public
    ...
    function AutoCompressedTextureFormats(const TextureURL: string): TTextureCompressions;
      ...
      try
        ...
        begin
          if Result.IndexOf(S[J]) <> -1 then
            OnWarning(wtMajor, 'MaterialProperties', Format('The texture URL "%s" is under the influence of more than one <auto_generated_textures> rule. Use <include> and <exclude> to avoid it',
              [S[J]]))
          else
            Result.Add(S[J]);
        end;
...
function TMaterialProperties.AutoCompressedTextureFormats(const TextureURL: string): TTextureCompressions;
var
  I: Integer;
begin
  Result := [];
  for I := 0 to FAutoGeneratedTexturesList.Count - 1 do
    if FAutoGeneratedTexturesList[I].TextureURLMatches(TextureURL) then
      Exit(FAutoGeneratedTexturesList[I].CompressedFormats);
end;
