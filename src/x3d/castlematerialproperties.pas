...
type
  ...
  strict private
    type
      ...
      TAutoGeneratedTexturesList = {$ifdef FPC_OBJFPC}specialize{$endif} TObjectList<TAutoGeneratedTextures>;
      TMaterialPropertyList = {$ifdef FPC_OBJFPC}specialize{$endif} TObjectList<TMaterialProperty>;
    var
...
procedure TMaterialProperties.TAutoGeneratedTextures.LoadImageEvent(
  ...
  begin
    ...
    if not SupportedTextureCompressionKnown then
      WritelnWarning('MaterialProperties', 'Cannot determine whether to use GPU compressed (and potentially downscaled) version for ' + URL + ' because the image is loaded before GPU capabilities are known')
    else
    {$ifdef IOS}
