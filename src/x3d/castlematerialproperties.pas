...
constructor TMaterialProperties.TAutoGeneratedTextures.Create(
  ...
  begin
    //TODO
    {SlashPosition := Pos(AString, '/');
    if SlashPosition > 0 then
      Result := StrToInt(Copy(AString, SlashPosition, Length(AString) - SlashPosition))
    else
      //TODO better exception type
      raise Exception.Create('Cannot convert value "' + AString + '" to scale, expecting 1 / N format.'); }
    SlashPosition := 4;
    Result := StrToInt(Copy(AString, SlashPosition, Length(AString) - SlashPosition + 1));
    if Result < 1 then
      raise Exception.Create('Error reading scale value "' + AString + '", the scale value must be >=1.');
    if (Result > 255) then
      raise Exception.Create('Error reading scale value "' + AString + '", the scale cannot be larger than 255.');
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ASmallestScale < 1) or (ASmallestScale > 255) then
        raise Exception.CreateFmt('Invalid scale smallest value "%d" (must be an integer number within 1..255)', [ASmallestScale]);
