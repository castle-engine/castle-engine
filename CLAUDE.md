...
## Web platform specific notes

When compiling for the web target:

1. We compile the engine and application Pascal code using the FPC for WebAssembly compiler.

2. We additionally compile a small "glue" application in `tools/build-tool/data/web/program_js.lpr` using `pas2js`. `pas2js` is a different compiler (more precisely _"transpiler"_, translating Pascal -> JavaScript). It compiles a (very very compatible) version of Pascal, but has a bit different set of libraries available.

See https://castle-engine.io/web for details how the web target works.

## Using JavaScript APIs from the Pascal code that is compiled with FPC for WebAssembly

JavaScript APIs are defined by a collection of `*.webidl` files inside `src/base_rendering/web/webidl/parts/` subdirectory. Don't just write these webidl files from scratch, instead download them from https://hg-edge.mozilla.org/mozilla-central/raw-file/tip/dom/webidl/ , like this:

```
cd src/base_rendering/web/webidl/parts/
wget https://hg-edge.mozilla.org/mozilla-central/raw-file/tip/dom/webidl/AudioContext.webidl
```

We edit these webidl files only if we have to,

- to make them handled by `webidl2pas`,
- and to avoid pulling in too many other webidl files as dependencies.

In the webidl files that we had to edit, we add a comment near the top:
...
Once the webidl files are ready, run the `src/base_rendering/web/webidl/mk_castlejobweb.sh` script to:

1. glue all `parts/*.webidl` into one big `castleinternaljobweb.webidl`

2. process `castleinternaljobweb.webidl` with `webidl2pas` utility to produce the `castleinternaljobweb.pas` unit file (called `CastleInternalJobWeb`, when using it in Pascal).

Do not edit the `castleinternaljobweb.pas` unit file by hand -- it has to be 100% auto-generated by the above process.

In the rest of engine code, e.g. in the `CastleInternalWebAudioBackend` unit, use classes (with their properties and methods) from the `CastleInternalJobWeb` unit.

- So do not invoke methods by writing method name as a string, like `AudioCtx.InvokeJSObjectResult('createGain' ...)`.

- Do not set property names by string, like `PannerNode.WriteJSPropertyUnicodeString('distanceModel', 'inverse')`.

- Instead use classes / interfaces from `CastleInternalJobWeb` in the engine code, and call their defined methods, access their defined properties.

Rules when using `CastleInternalJobWeb` API:

- The JavaScript class like `Xxx` is declared as interface `IJSXxx` in Pascal (use interface for almost all operations). Like this: `GL: IJSWebGLRenderingContext;`

- When creating an instance of it, use the class name to access the constructor, like `GL := TJSWebGLRenderingContext.Create;`

- Cast one JavaScript class to another using `Cast()` (not Pascal operator `as`). Never call this on potentially `nil` instance, as it would crash.

- To check for JavaScript `null`, check `VarIsNull(xxx)` or compare with `Variants.Null`. (Do not check `xxx = nil`, as Pascal `nil` is not used to express null JavaScript values.) See https://wiki.lazarus.freepascal.org/WebAssembly/DOM#Function_Result .

This is correct example:

```pascal
var
  DivLoadingInfoBase: IJSElement;
  DivLoadingInfo: IJSHTMLDivElement;
begin
  DivLoadingInfoBase := JSDocument.getElementById('castle-loading-info');
  if DivLoadingInfoBase = nil then // check nil before using Cast() on it
    Exit;
  DivLoadingInfo := TJSHTMLDivElement.Cast(DivLoadingInfoBase);
...
```

## Using JavaScript APIs from the Pascal code (that is compiled using pas2js)

NOTE: This section concerns the Pascal code that is compiled using `pas2js`, which is right now just `tools/build-tool/data/web/program_js.lpr`.

- In code compiler with pas2js, JavaScript class `Xxx` corresponds to the Pascal class `TJSXxx`. There is no interface.

- Create it with `TJSXxx.New` (not `Create`), like this:
...
- Cast by `as`, compare with `nil`, as usual for Pascal classes.
