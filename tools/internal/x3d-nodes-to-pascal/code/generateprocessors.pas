...
type
  ...
  TX3DNodeInformation = class
    ...
    Vrml1: Boolean;
    { Node is a Castle Game Engine extension (or, in general,
  ...
  TX3DFieldInformation = class
    ...
    Angle: Boolean;
    WeakLink: Boolean;
    { Field or event is a Castle Game Engine extension (or, in general,
      not part of the X3D specification), while the node that contains it
      is standard. }
    IsExtension: Boolean;
  ...
  strict private
    ...
    { Body of the generated GetExtensions method override.
      If it remains '' (no extensions in this node), we don't generate
      the GetExtensions override at all. }
    OutputGetExtensions: String;
  public
...
var
  ...
  VersionForAlternativeName: Integer;
begin
  FieldConfigure := '';
  ...
  begin
    ...
    begin
      ...
      for VersionForAlternativeName := 0 to 4 do
        if Field.AlternativeNames[VersionForAlternativeName] <> '' then
          FieldConfigure += '   ' + Field.PascalNamePrefixed + '.AddAlternativeName(''' + Field.AlternativeNames[VersionForAlternativeName] + ''', ' + IntToStr(VersionForAlternativeName) + ');' + NL;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if (Tokens.Count = 2) and
             (Tokens[0] = 'node-is-extension:') then
          begin
            ValidatePerNodeCommand('node-is-extension');
            Node.IsExtension := StrToBool(Tokens[1]);
          end else
          ...
          if (Tokens.Count = 2) and
             (Tokens[0] = 'is-extension:') then
          begin
            ValidatePerFieldCommand('is-extension');
            LastField.IsExtension := StrToBool(Tokens[1]);
          end else

          if (Tokens.Count = 3) and
             (Tokens[0] = 'alternative-name:') then
          begin
            ValidatePerFieldCommand('alternative-name');
            LastField.AlternativeNames[StrToInt(Tokens[2])] := Tokens[1];
          end else
...
procedure THelperProcessor.NodeField(const Node: TX3DNodeInformation;
  ...
  { Accumulate the body of GetExtensions override to OutputGetExtensions.
    Note: Do this before any early Exits below (they only decide
    about helper getters/setters, while extension detection
    should account for every extension field/event). }
  procedure ExtendOutputGetExtensions;
  begin
    if Field.IsExtension then
    begin
      if Field.AccessType in [atInputOnly, atOutputOnly] then
        { Extension event is "used" when some ROUTE is connected to it. }
        OutputGetExtensions +=
          Field.ConditionsBegin +
          '  if ' + Field.PascalNamePrefixed + '.InternalRouteCount > 0 then' + NL +
          '    ExtensionNames.Add(X3DType + ''.'' + ' + Field.PascalNamePrefixed + '.X3DName);' + NL +
          Field.ConditionsEnd
      else
        { Extension field is "used" when it has a non-default value
          (the same logic that decides whether to save the field
          to file, see TX3DField.FieldSaveToStream). }
        OutputGetExtensions +=
          Field.ConditionsBegin +
          '  if not ' + Field.PascalNamePrefixed + '.EqualsDefaultValue then' + NL +
          '    ExtensionNames.Add(X3DType + ''.'' + ' + Field.PascalNamePrefixed + '.X3DName);' + NL +
          Field.ConditionsEnd;
    end;
  end;
...
begin
  ...
  { Generate GetExtensions override, if this node is an extension
    or has any extension fields / events. }
  if Node.IsExtension then
    OutputGetExtensions :=
      '  ExtensionNames.Add(X3DType);' + NL +
      OutputGetExtensions;
  if OutputGetExtensions <> '' then
  begin
    OutputPublicInterface +=
      '    procedure GetExtensions(const ExtensionNames: TStrings); override;' + NL;
    OutputImplementation +=
      'procedure ' + Node.PascalType + '.GetExtensions(const ExtensionNames: TStrings);' + NL +
      'begin' + NL +
      '  inherited;' + NL +
      OutputGetExtensions +
      'end;' + NL +
