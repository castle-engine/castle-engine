...
type
  ...
  TX3DFieldInformation = class
    ...
    SetterBefore: String;
    { Place more information inside THelperProcessor output:
      define field, property, initialize them in CreateNode.
...
var
  EventInOrOut: string;
  FieldImplementationComment: String;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          { per-node commands }
          if (Tokens.Count = 2) and
             (Tokens[0] = 'auto-generate-more:') then
          begin
            ValidatePerNodeCommand('auto-generate-more');
            Node.AutoGenerateMore := StrToBool(Tokens[1]);
          end else
          ...
          if (Tokens.Count = 2) and
             (Tokens[0] = 'field-generate-more:') then
          begin
            ValidatePerFieldCommand('field-generate-more');
            LastField.GenerateMore := StrToBool(Tokens[1]);
          end else
...
begin
  if Node.AutoGenerateMore and Field.GenerateMore then
