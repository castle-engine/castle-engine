...
type
  ...
  TX3DFieldInformation = class
    ...
    function GetChangeAlways: String;
...
begin
  ...
  begin
    ...
    begin
      ...
      { TODO: Field.Comment may contain information not from X3D spec at all,
        and not interested in Pascal, like change: xxx, enumerated-type: xxx . }
...
function TX3DFieldInformation.GetChangeAlways: String;
var
  Tokens: TStringList;
  StartToken: Integer;
begin
  Result := 'chVisibleNonGeometry';
  Tokens := CreateTokens(Comment);
  try
    StartToken := Tokens.IndexOf('change:');
    if StartToken <> -1 then
      Result := Tokens[StartToken + 1];
...
procedure TProcessor.ProcessFile(const InputFileName: string);
  ...
  begin
    ...
    Field.ChangeAlways := Field.GetChangeAlways;
    ...
    Field.NotSlim := Pos('NOT-SLIM', Field.Comment) <> 0;
  end;
