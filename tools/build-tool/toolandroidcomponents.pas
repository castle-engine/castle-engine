...
var
    ...
    begin
      if SourceAttribs[I].NodeType <> ATTRIBUTE_NODE then
        raise ECannotMergeManifest.Create('Attribute node does not have NodeType = ATTRIBUTE_NODE: ' +
          UTF8Encode(SourceAttribs[I].NodeName));
      // if Verbose then
  ...
  begin
    if DestinationXml.DocumentElement.ChildElement(UTF8Encode(SourceElement.TagName), false) <> nil then
      raise ECannotMergeManifest.Create(
        'Cannot merge AndroidManifest.xml, only one <' + UTF8Encode(SourceElement.TagName) + '> is allowed');
      ...
      try
        ...
        begin
          ...
          if I.Current.TagName = 'supports-screens' then
            ...
            raise ECannotMergeManifest.Create('Cannot merge AndroidManifest.xml element <' + UTF8Encode(I.Current.TagName) + '>');
        end;
      finally FreeAndNil(I) end;
