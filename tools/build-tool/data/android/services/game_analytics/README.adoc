...
* This service requires Android min SDK version 26 or higher. The build tool raises the project's `min_sdk_version` (at the `<android>` element in link:pass:[CastleEngineManifest.xml examples][CastleEngineManifest.xml]) to 26 automatically when this service is used, so you don't have to do anything.
