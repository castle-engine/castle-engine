...
public class ServiceGooglePlayGames extends ServiceAbstract
{
    ...
    // created on-demand when trying to connect
    ...
    */
    private void signIn()
    {
        ...
        if (gamesSignInClient == null) {
            ...
            gamesSignInClient = PlayGames.getGamesSignInClient(a);
        }
        signInSilently(true);
    }

    /* Check are we authenticated.
       - If yes, call successfullSignIn.
       - If no, do nothing (if interactiveIfFails == false) or
         start interactive sign-in (if interactiveIfFails == true).

       Similar to sample on https://developer.android.com/games/pgs/android/migrate-to-v2

       Call only if gamesSignInClient != null.
    */
    private void signInSilently(boolean interactiveIfFails)
    {
    ...
    /* Start interactive sign-in process.
       Call successfullSignIn() if it was OK.

       Following https://developer.android.com/games/pgs/android/migrate-to-v2  */
    ...
    private void signOutClicked()
    {
       ...
       /* There is no sign-out in v2 API,
          see https://developer.android.com/games/pgs/android/migrate-to-v2 */
       logWarning(CATEGORY, "Cannot sign out from Google Play Games in API v2. Changing our state anyway, to not communicate with Google Play Games.");
    ...
    {
        final Activity a = getActivity();
        PlayGames.getAchievementsClient(a)
    ...
    {
        final Activity a = getActivity();
        PlayGames.getAchievementsClient(a).unlock(achievementId);
    }
    ...
    private void showSaveGames(final String title, final boolean allowAddButton, final boolean allowDelete, final int maxNumberOfSaveGamesToShow)
    {
        ...
        final Activity a = getActivity();
        SnapshotsClient snapshotsClient = PlayGames.getSnapshotsClient(a);
    ...
    {
        final Activity a = getActivity();
        PlayGames.getLeaderboardsClient(a)
    ...
    {
        final Activity a = getActivity();
        LeaderboardsClient leaderboardsClient = PlayGames.getLeaderboardsClient(a);
