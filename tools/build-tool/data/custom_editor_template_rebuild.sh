...
EDITOR_SOURCE_CODE=../../castle-editor/code/

cp -f "${EDITOR_SOURCE_CODE}castle_editor.ico" custom_editor_template/

cp -f "${EDITOR_SOURCE_CODE}castle_editor.lpr" custom_editor_template/
sed --in-place \
...
cp -f "${EDITOR_SOURCE_CODE}castle_editor.lpi" custom_editor_template/
sed --in-place \
  ...
  -e 's|<Filename Value="\([_a-zA-Z0-9]\+.pas\)"|<Filename Value="${CASTLE_ENGINE_PATH}tools/castle-editor/code/\1"|' \
  -e 's|<Filename Value="../../common-code/|<Filename Value="${CASTLE_ENGINE_PATH}tools/common-code/|' \
  -e 's|<Filename Value="../castle-editor"/>|<Filename Value="castle-editor"/>|' \
  -e 's|<IncludeFiles Value="../../../src/base;$(ProjOutDir)"/>|<IncludeFiles Value="${CASTLE_ENGINE_PATH}src/base;$(ProjOutDir)"/>|' \
  -e 's|<OtherUnitFiles Value="../../common-code"/>|<OtherUnitFiles Value="${CASTLE_ENGINE_PATH}tools/castle-editor/code;${CASTLE_ENGINE_PATH}tools/common-code"/>|' \
