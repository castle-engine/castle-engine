...
begin
  case OptionNum of
    ...
    13: ;
    14: ChangeProjectDir(Argument);
    15: PackageFormat := StringToPackageFormat(Argument);
    16: PackageNameIncludeVersion := false;
    17: UpdateOnlyCode := true;
    18: IosSimulatorSupport := true;
    19: CleanAll := true;
    20: ManifestName := Argument;
    21: WaitForProcessId := StrToInt64(Argument);
    22: GuiErrors := true;
    23: OverrideCompiler := StringToCompiler(Argument);
    24: GuidFromName := true;
    25: {$ifdef MSWINDOWS} ForcePipesPassthrough := true {$endif};
