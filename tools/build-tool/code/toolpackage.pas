...
type
  ...
  { Package a project to a directory, excluding files not specific to target platform. }
  TPackageDirectoryPlatformSpecific = class(TPackageDirectory)
  strict private
  ...
  public
    ...
    constructor Create(const ATopDirectoryName: string;
      const TargetPlatform: TCastlePlatform; const ProjectDataPath: String);
    destructor Destroy; override;
    procedure Add(const SourceFileName, DestinationFileName: string); override;
  end;
...
{ TPackageDirectoryPlatformSpecific ------------------------------------------ }

constructor TPackageDirectoryPlatformSpecific.Create(
  const ATopDirectoryName: string; const TargetPlatform: TCastlePlatform;
  const ProjectDataPath: String);
var
  AutoGeneratedUrl: String;
begin
  inherited Create(ATopDirectoryName);
  FTargetPlatform := TargetPlatform;

  AutoGeneratedUrl := FilenameToURISafe(ProjectDataPath + TAutoGenerated.FileName);
  AutoGenerated := TAutoGenerated.Create;
  AutoGenerated.LoadFromFile(AutoGeneratedUrl);
end;

destructor TPackageDirectoryPlatformSpecific.Destroy;
begin
  FreeAndNil(AutoGenerated);
  inherited;
end;

procedure TPackageDirectoryPlatformSpecific.Add(const SourceFileName, DestinationFileName: string);
var
  DestinationInData: String;
begin
  DestinationInData := StringReplace(DestinationFileName, '\', '/', [rfReplaceAll]);
  if IsPrefix('data/', DestinationInData, true) then
  begin
    DestinationInData := PrefixRemove('data/', DestinationInData, true);
    if not AutoGenerated.IncludeDataForPlatform(FTargetPlatform, DestinationInData) then
    begin
      WritelnVerbose(Format('Not packaging "%s" because it is not for the platform "%s"', [
        DestinationFileName,
        PlatformToStr(FTargetPlatform)
      ]));
      Exit;
    end;
  end;

  inherited Add(SourceFileName, DestinationFileName);
end;
